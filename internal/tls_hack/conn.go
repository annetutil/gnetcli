...
Again:
	...
	switch typ {
	default:
		err := c.in.setErrorLocked(c.sendAlert(alertUnexpectedMessage))
		if err != nil {
			return err
		}
	...
	case recordTypeAlert:
		...
		if alert(data[1]) == alertCloseNotify {
			err := c.in.setErrorLocked(io.EOF)
			if err != nil {
				return err
			}
			break
		...
		case alertLevelError:
			err := c.in.setErrorLocked(&net.OpError{Op: "remote error", Err: alert(data[1])})
			if err != nil {
				return err
			}
		default:
	...
	case recordTypeChangeCipherSpec:
		...
		if err != nil {
			err := c.in.setErrorLocked(c.sendAlert(err.(alert)))
			if err != nil {
				return err
			}
		}
