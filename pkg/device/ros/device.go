...
const (
	...
	errorExpression    = `(\r|^)(bad command name.*\(line \d+ column \d+\).*$|syntax error.*\(line \d+ column \d+\).*$|\[(?P<question>Safe mode released by another user)\]|expected end of command \(line \d+ column \d+\)|failure: duplicate address)`
